...
from functools import partial #permite poner el parametro en la funcion del command
...
class Calculadora(object):
    ...
    def __init__(self):
        ...
        ttk.Button(mainframe, text='/', command=partial(self.ponerNUMERO, '/')).grid(column=3, row=7, sticky=W)
        ttk.Button(mainframe, text='=', command=partial(self.ponerOPERADOR, '=')).grid(column=2, row=8, sticky=(W,E), columnspan=2)
        ttk.Button(mainframe, text='DEL', command=self.borrarPanel).grid(column=1, row=8, sticky=W)
        self.__panel.set('')
        panelEntry.focus()
